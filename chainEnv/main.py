...
def chains_py_init():
    ...
    print(chains_data)
    with open("../PythonMiddlewarebase/chains.py", "w") as file:
        file.write(chains_data)

def cli_wallet_start():
    chainID_init()
    cmd = 'cd wallet; ./cli_wallet --chain-id ' + chainID + ' -s ws://127.0.0.1:8049'
    print('[cli_wallet_start] cmd : ' + cmd)
    os.system(cmd)
...
def middleware_install():
    chains_py_init()
    os.system('cd ..; python3 setup.py install --user')
